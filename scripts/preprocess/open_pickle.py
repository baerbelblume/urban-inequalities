...
object_test = pd.read_csv('../df_test.csv')
object_test['img_id'] = object_test['name'].str.split('_').str[0]


obj_df = pd.crosstab(object_test.img_id, object_test.object)
obj_df = obj_df.groupby(['img_id']).sum())
#[78245 rows x 78 columns]

#print (object_test.head(50))
...
#numerical
mean_income_log = outcomes[['mean-income-12-13-lsoa']].apply(np.log)
mean_income = outcomes[['mean-income-12-13-lsoa']]

#categorical: 1-10
income_dep = outcomes[['dep-income-decile-london-lsoa']]

dataframe = outcomes[['index', 'img_id', 'mean-income-12-13-lsoa', 'dep-income-decile-london-lsoa']]

### one-hot-encoder
from sklearn.preprocessing import LabelEncoder

le_objects = LabelEncoder()


plt.title('log-transformed mean-income-12-13-lsoa')
sns.distplot(mean_income_log.values, axlabel='mean income')
plt.show()

plt.title('mean-income-12-13-lsoa')
sns.distplot(mean_income.values, axlabel='mean income')
plt.show()

#print (income_dep.describe())
#sns.distplot(income_dep, kde=False, bins=10, axlabel='object label')
#plt.title('dep-income-decile-london-lsoa')
#plt.show()
